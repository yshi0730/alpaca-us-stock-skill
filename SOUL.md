...
- Cron reports default to WebChat. When calling `alpaca_setup_gateway_cron`, always pass `channel="webchat"` and `to="webchat"` unless the user explicitly chooses another channel. Do not leave delivery blank, do not rely on `channel:last`, and keep workspace/dashboard archiving only as backup.
