...
1. Run **one command**: `bash dashboard/setup.sh`. It is idempotent and
   does the whole infra bring-up — clones/pulls Layer 0
   (claw-dashboard-skill), installs deps, copies the hub, registers the
   device tunnel, starts hub + cloudflared only if not already running,
   and renders the page. Do NOT hand-run the multi-step setup, do NOT
   build generic widgets. Relay its final status block (URL) to the user.
